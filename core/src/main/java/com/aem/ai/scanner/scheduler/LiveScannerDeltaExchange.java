...
public class LiveScannerDeltaExchange implements Runnable {

    private static final Logger log = LoggerFactory.getLogger(LiveScannerDeltaExchange.class);

    @ObjectClassDefinition(name = "BSK DeltaExchange Live Scanner Scheduler",
            description = "Fetch market data for  DeltaExchange symbols on a schedule (pure OSGi, no threads)")
    public @interface Config {
        ...
        String scheduler_name() default "LiveScannerDeltaExchange";
    ...
    protected void deactivate() {
        log.info("🛑 LiveScannerDeltaExchange deactivated.");
    }
    private static final String SCHEDULER_KEY = "LiveScannerDeltaExchange";

    @Override
    public void run() {
        try {
            log.info("💡 LiveScannerDeltaExchange scheduler triggered at {}", LocalDateTime.now());
            ...
            log.error("❌ Unhandled error in LiveScannerDeltaExchange scheduler (kept alive): {}", t.getMessage(), t);
        ...
        try {
            ...
            for (TradeModel t : openTrades) {
                ...
                if (hitTarget || hitStop) {
                    ...
                    telegram.sendMessageDailyCryptoAlerts("Closed trade: " + symbol + " pnl=" + pnl);
